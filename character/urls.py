...
urlpatterns = [
    ...
    path('character/detail/<int:pk>', character_detail, name='character_detail'),  # 캐릭터 디테일 페이지
    ...
    path('character/update/<int:pk>', character_update, name='character_update'),
    # 캐릭터 이름만 수정                   {{ 차이 }} --> 캐릭터의 이름은 감정 그 자체로 해서 변화시킬 수 없는것으로 구상한것이었음
    ...
    path('trash/restore/<int:pk>', trash_restore, name='trash_restore'),
    # 여기는 휴지통에서 캐릭터 선택하면 복구하는 페이지 만약 복구했는데 캐릭터 5개이면 복구못하게

    path('trash/perfect/delete/<int:pk>', trash_perfect_delete, name='trash_perfect_delete'),
    # 이거는 캐릭터 영구 삭제 url 즉 휴지통에서만 작동할수 있는 url임
